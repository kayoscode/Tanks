...
var Moves = {
    ...
    aromatherapy: {
        ...
        onHit: function (target, source, move) {
            ...
            var success = false;
            var allies = __spreadArray(__spreadArray([], target.side.pokemon, true), ((_a = target.side.allySide) === null || _a === void 0 ? void 0 : _a.pokemon) || [], true);
            for (var _i = 0, allies_1 = allies; _i < allies_1.length; _i++) {
                var ally = allies_1[_i];
    ...
    beakblast: {
        ...
        priorityChargeCallback: function (pokemon) {
    ...
    courtchange: {
        ...
        onHitField: function (target, source) {
            ...
            if (this.gameType === "freeforall") {
                ...
                for (var i = 0; i < 4; i++) {
                    var sourceSideConditions = temp[sides[i].n];
                    var targetSide = sides[(i + offset) % 4]; // the next side in rotation
                    for (var id in sourceSideConditions) {
                        targetSide.sideConditions[id] = sourceSideConditions[id];
                        var effectName = this.dex.conditions.get(id).name;
                        var layers = sourceSideConditions[id].layers || 1;
                        for (; layers > 0; layers--)
                ...
                var sourceSideConditions = source.side.sideConditions;
                var targetSideConditions = source.side.foe.sideConditions;
                var sourceTemp = {};
                var targetTemp = {};
                for (var id in sourceSideConditions) {
                    if (!sideConditions.includes(id))
                ...
                for (var id in targetSideConditions) {
                    if (!sideConditions.includes(id))
                        continue;
                ...
                for (var id in sourceTemp) {
                    targetSideConditions[id] = sourceTemp[id];
                }
                for (var id in targetTemp) {
    ...
    fly: {
        ...
        onTryMove: function (attacker, defender, move) {
            ...
            if (this.gameType === 'doubles' || this.gameType === 'multi') {
                var animatedTarget = attacker.getMoveTargets(this.dex.getActiveMove('aerialace'), defender).targets[0];
                if (animatedTarget) {
                    this.hint("".concat(move.name, "'s animation targeted ").concat(animatedTarget.name));
                }
            }
            attacker.addVolatile('twoturnmove', defender);
            return null;
    ...
    focuspunch: {
        ...
        beforeMoveCallback: function (pokemon) {
            var _a;
            if ((_a = pokemon.volatiles['focuspunch']) === null || _a === void 0 ? void 0 : _a.lostFocus) {
    ...
    gmaxmeltdown: {
        ...
        isMax: "Melmetal",
        self: {
            onHit: function (source, target, effect) {
                for (var _i = 0, _a = source.foes(); _i < _a.length; _i++) {
                    var pokemon = _a[_i];
    ...
    gmaxsteelsurge: {
        ...
        condition: {
            ...
            onEntryHazard: function (pokemon) {
    ...
    healbell: {
        ...
        onHit: function (target, source) {
            ...
            var success = false;
            var allies = __spreadArray(__spreadArray([], target.side.pokemon, true), ((_a = target.side.allySide) === null || _a === void 0 ? void 0 : _a.pokemon) || [], true);
            for (var _i = 0, allies_2 = allies; _i < allies_2.length; _i++) {
                var ally = allies_2[_i];
    ...
    iondeluge: {
        ...
        condition: {
            duration: 1,
            onFieldStart: function (target, source, sourceEffect) {
                this.add('-fieldactivate', 'move: Ion Deluge');
                this.hint("Normal-type moves become Electric-type after using ".concat(sourceEffect, "."));
            },
            onModifyTypePriority: -2,
            onModifyType: function (move) {
                if (move.type === 'Normal') {
    ...
    metronome: {
        ...
        onHit: function (target, source, effect) {
            var _this = this;
            var moves = this.dex.moves.all().filter(function (move) { return ((![2, 4].includes(_this.gen) || !source.moves.includes(move.id)) &&
    ...
    moonlight: {
        ...
        onHit: function (pokemon) {
            ...
            var success = !!this.heal(this.modify(pokemon.maxhp, factor));
            if (!success) {
    ...
    rapidspin: {
        ...
        onAfterHit: function (target, pokemon) {
            ...
            for (var _i = 0, sideConditions_1 = sideConditions; _i < sideConditions_1.length; _i++) {
                var condition = sideConditions_1[_i];
        ...
        onAfterSubDamage: function (damage, target, pokemon) {
            ...
            var sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge'];
            for (var _i = 0, sideConditions_2 = sideConditions; _i < sideConditions_2.length; _i++) {
                var condition = sideConditions_2[_i];
                if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
                    this.add('-sideend', pokemon.side, this.dex.conditions.get(condition).name, '[from] move: Rapid Spin', '[of] ' + pokemon);
                }
            }
    ...
    rest: {
        ...
        onHit: function (target, source, move) {
            var result = target.setStatus('slp', source, move);
            if (!result)
                return result;
    ...
    risingvoltage: {
        ...
        basePowerCallback: function (source, target, move) {
            if (this.field.isTerrain('electricterrain') && target.isGrounded()) {
                if (!source.isAlly(target))
                    this.hint("".concat(move.name, "'s BP doubled on grounded target."));
                return move.basePower * 2;
            }
            return move.basePower;
        },
    ...
    swallow: {
        ...
        onHit: function (pokemon) {
            ...
            var success = !!this.heal(this.modify(pokemon.maxhp, healAmount[(pokemon.volatiles['stockpile'].layers - 1)]));
            if (!success)
                this.add('-fail', pokemon, 'heal');
            pokemon.removeVolatile('stockpile');
    ...
    torment: {
        ...
        condition: {
            ...
            onStart: function (pokemon, source, effect) {
                ...
                if ((effect === null || effect === void 0 ? void 0 : effect.id) === 'gmaxmeltdown')
        ...
        condition: {
            ...
            onModifyMove: function (move, source, target) {
                ...
                if (!move.overrideOffensiveStat)
                    return;
                var statAndBoosts = move.overrideOffensiveStat;
                if (!['def', 'spd'].includes(statAndBoosts))
                    return;
                move.overrideOffensiveStat = statAndBoosts === 'def' ? 'spd' : 'def';
                this.hint("".concat(move.name, " uses ").concat(statAndBoosts === 'def' ? '' : 'Sp. ', "Def boosts when Wonder Room is active."));
            },
            onFieldStart: function (field, source) {
