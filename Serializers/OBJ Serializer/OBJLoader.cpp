...
static bool loadFloat(std::string& line, int& index, float& flt) {
    ...
    index += (int)newIndex;
    return index != (int)newIndex;
}

static bool loadFloats(std::string& line, std::vector<float>& list, uint32_t count) {
    int index = 2;
...
static bool loadFace(std::string& line, std::vector<int>& indices, std::vector<float>& positions, std::vector<float>& normals, 
    ...
    if(vertices.find(vertex1) == vertices.end()) {
        vertices[vertex1] = verticesCount++;
    }

    if(vertices.find(vertex2) == vertices.end()) {
        vertices[vertex2] = verticesCount++;
    }

    if(vertices.find(vertex3) == vertices.end()) {
        vertices[vertex3] = verticesCount++;
    }
...
bool ModelLoader::loadOBJ(const std::string& filePath, IndexedModel& model) {
    std::ifstream file(filePath);
    std::string line = "";

    // If the file cannot be found, obviously it cannot load it.
    if(!file.is_open()) {
        return false;
    }
    ...
    model.positionsCount = (int)vertices.size() * 3;
    model.normalsCount = (int)vertices.size() * 3;
    model.uvsCount = (int)vertices.size() * 2;
